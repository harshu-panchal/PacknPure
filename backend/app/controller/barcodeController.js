 ...
 *
 * Query:
 *  - newlyCreatedOnly=true  (seller: only barcodes created since `since` or last 1 hour)
 *  - since=ISO date
 *  - variantIds=id1,id2
 */
export const downloadBarcodePdf = async (req, res) => {
  try {
    ...
    // Ensure missing barcodes exist before PDF (non-destructive).
    ...
    if (newlyCreatedOnly && !since && !variantIds?.length) {
      // Default window for "newly created seller barcodes"
 ...
 * Optional PNG preview of a Code128 barcode (owner must own a matching variant).
 */
