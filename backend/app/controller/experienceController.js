...
const validateBasePayload = async (body) => {
  ...
  if (pageType === "header") {
    ...
    const anchor = await Category.findOne({
      _id: headerId,
      type: "category",
      parentId: null,
    });
    if (!anchor) {
      throw new Error("Invalid category id for storefront page");
    }
  }
...
const validateAndNormalizeConfig = async (displayType, config = {}) => {
  ...
  if (displayType === "banners") {
    ...
    normalized.banners = {
      items: items
        .filter((b) => b && (b.imageUrl || b.mobileImageUrl))
        .map((b) => ({
          imageUrl: b.imageUrl || "",
  ...
  try {
    ...
    const payload = config
      ? {
          ...
          promoBanner: config.promoBanner || null,
          categoryIds: config.categoryIds || [],
        }
      : { banners: { items: [] }, promoBanner: null, categoryIds: [] };
...
export const getAdminHeroConfig = async (req, res) => {
  try {
    ...
    return handleResponse(
      ...
      200,
      "Hero config fetched",
      config || { banners: { items: [] }, promoBanner: null, categoryIds: [] }
    );
  } catch (error) {
    return handleResponse(res, 500, error.message);
  }
...
export const upsertHeroConfig = async (req, res) => {
  try {
    const { pageType, headerId, banners, promoBanner, categoryIds } = req.body;
    ...
    if (pageType === "header") {
      ...
      if (!anchor) {
        return handleResponse(res, 400, "Invalid category id for storefront page");
      }
    }
    ...
    if (promoBanner !== undefined) {
      if (promoBanner && promoBanner.imageUrl) {
        update.promoBanner = {
          ...
          cta: promoBanner.cta || "",
          linkType: promoBanner.linkType || "none",
          linkValue: promoBanner.linkValue || "",
          status: promoBanner.status || "active",
        };
      } else {
        update.promoBanner = null;
      }
    }
