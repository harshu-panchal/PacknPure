...
export const isAccountVerified = async (req, res, next) => {
  try {
    ...
    if (role === "seller" && account.isActive === false) {
      return handleResponse(
        res,
        403,
        "Your seller account is inactive. Contact admin to restore access.",
      );
    }
    ...
    if (!account.isVerified) {
      const pendingMessage =
        role === "seller"
          ? "Your seller profile is not approved yet. You cannot add products or catalog listings until admin verifies your account."
          : "Your account is pending admin approval. You cannot perform this action yet.";
      return handleResponse(res, 403, pendingMessage);
    }
